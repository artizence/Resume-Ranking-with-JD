...
def open_docx_file(file_name):
    text = preprocess(docx2txt.process(file_name))
    return text 

##lets become a awersome version of yourself
def resume_reader(file_name):

    try:
        try:
            resume = open_pdf_file(file_name)
        except:
            resume = open_docx_file(file_name)
        
        return resume
    except:
        print('Check the format of the file only .docx and .pdf are accepted')
        return 'ERROR'    
